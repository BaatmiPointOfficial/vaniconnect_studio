...
export default function Studio() {
 ...
 const handleVideoEnhanceProcess = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/api/enhance-video`, formData);
      
      if (response.data && response.data.job_id) {
          checkJobStatus(response.data.job_id);
      } else if (response.data && (response.data.file_name || response.data.file_url)) {
          // 2. Point the download to your local engine
          const finalUrl = response.data.file_url 
              ? response.data.file_url 
              : `${import.meta.env.VITE_API_URL}/downloads/${response.data.file_name}`;
      ...
      console.error(e); // Added so errors show up in your browser console!
      setLoading(false); 
    }
  };

 const handleVideoWatermarkProcess = async () => {
    if (!selectedFile) return;
    setLoading(true);
    setResultUrl(null);
    const formData = new FormData();
    ...
    try {
      // 1. Point to your local Python engine
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/api/remove-video-watermark`, formData);
      
      if (response.data && response.data.job_id) {
        checkJobStatus(response.data.job_id);
      } else if (response.data && (response.data.file_name || response.data.file_url)) {
        // 2. Fetch the download from your local engine
        const finalUrl = response.data.file_url 
            ? response.data.file_url 
            : `${import.meta.env.VITE_API_URL}/downloads/${response.data.file_name}`;
        setResultUrl(finalUrl);
        ...
        alert("Backend did not return a file!"); 
        setLoading(false); 
      }
    } catch (e) { 
 ...
 const handlePhotoWatermarkProcess = async () => {
    ...
    try {
      // 1. Point to your local Python engine
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/api/remove-photo-watermark`, formData);
  ...
  const handlePhotoEnhanceProcess = async () => {
    ...
    try {
      // 1. Point the upload to your local Python engine
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/api/enhance-photo`, formData);
  ...
  const handleYTDownload = async () => {
    ...
    try {
        ...
        // 1. Point straight to your LOCAL Python engine! Bye bye Render! 🚀
        const response = await axios.post(`${import.meta.env.VITE_API_URL}/api/youtube-downloader`, formData);

        if (response.data && response.data.file_name) {
            // 2. Grab the file straight from your local downloads folder
            const finalUrl = `${import.meta.env.VITE_API_URL}/downloads/${response.data.file_name}`;
        ...
        console.error(error);
        alert("Error processing YouTube link. Check your python terminal!");
        setLoading(false);
    }
  };
  ...
  const handleClipCutProcess = async () => {
    if (!selectedFile) {
    ...
    // 1. Pack the data exactly how your Python backend expects it
    const formData = new FormData();
    formData.append("file", selectedFile);
    ...
    try {
      ...
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/api/clipcut-pro`, formData);
      
      if (response.data && response.data.job_id) {
        checkJobStatus(response.data.job_id);
      } else if (response.data && (response.data.file_name || response.data.file_url)) {
        // 3. Catch the downloaded file
        const finalUrl = response.data.file_url 
            ? response.data.file_url 
            : `${import.meta.env.VITE_API_URL}/downloads/${response.data.file_name}`;
...
const handleAddLogoProcess = async () => {
    ...
    formData.append("logo_file", selectedLogo);  
    formData.append("x", logoX);
    formData.append("y", logoY);
    formData.append("logo_w", logoW);            

    try {
      // 1. Point the upload to your local engine
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/api/add-custom-logo`, formData);
      ...
      const errorMsg = e.response && e.response.data ? JSON.stringify(e.response.data) : e.message;
      alert(`Real Error: ${errorMsg}`);
      console.error(e); // Let's log it to the console too!
      setLoading(false); 
    }
  };
        ...
        <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-lg border p-10 text-center">
          ...
          {!loading && !resultUrl && (
            <div className="mb-8 max-w-xl mx-auto">
             ...
             {/* 1. THIS IS THE START BUTTON (Talks to the AI/Backend) */}
          ...
          {resultUrl && !loading && (
            <div className="animate-in zoom-in duration-300 mt-8">
              ...
              {/* 2. THIS IS THE SAVE BUTTON (Uses handleForceDownload to save to PC!) */}
              <button
                type="button"
                onClick={() => handleForceDownload(resultUrl.startsWith('http') ? resultUrl : `https://vaniconnect-vaniconnect-api.hf.space${resultUrl}`)} 
                className="bg-red-500 text-white px-8 py-4 rounded-xl font-bold inline-flex items-center shadow-lg hover:bg-red-600 text-lg"
              >
        ...
        <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-lg border p-10">
          ...
          {selectedFile && selectedLogo && !loading && !resultUrl && (
            <div className="text-center">
              {/* 1. WE CHANGED THIS BACK TO THE START BUTTON! */}
              <button 
                onClick={handleAddLogoProcess} 
                className="bg-indigo-600 text-white px-8 py-4 rounded-xl font-bold shadow-lg hover:bg-indigo-700 text-lg"
              >
                Start Branding Video
              </button>
            </div>
          )}
          ...
          {resultUrl && (
            <div className="mt-10 text-center">
              ...
              {/* 2. WE PUT THE NEW DOWNLOAD BUTTON HERE AT THE BOTTOM! */}
              <button 
                onClick={() => handleForceDownload(resultUrl)} 
                className="bg-green-500 text-white px-8 py-4 rounded-xl font-bold inline-flex items-center"
              >
...
>
        ...
        <div className="max-w-3xl mx-auto bg-white rounded-2xl shadow-lg border p-10 text-center">
          ...
          {selectedFile && !loading && !resultUrl && (
            <div>
             ...
             <button 
                onClick={handlePhotoEnhanceProcess} 
                className="bg-green-500 text-white px-10 py-4 rounded-xl font-bold shadow-lg hover:bg-green-600 text-lg transition-transform active:scale-95"
              >
                Enhance Photo
              </button>
            </div>
          ...
          {resultUrl && !loading && (
            <div className="animate-in zoom-in duration-300">
              ...
              <button 
                onClick={() => handleForceDownload(resultUrl.startsWith('http') ? resultUrl : `https://vaniconnect-vaniconnect-api.hf.space${resultUrl}`)} 
                className="bg-green-500 text-white px-8 py-4 rounded-xl font-bold inline-flex items-center shadow-lg hover:bg-green-600"
              >
        ...
        <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-lg border p-10 text-center">
          ...
          {selectedFile && !loading && !resultUrl && (
            <div>
              ...
              <div className="bg-teal-50 p-6 rounded-xl border border-teal-200 mb-8 max-w-2xl mx-auto">
               ...
               {/* 1. WE RESTORED THE START BUTTON HERE! (Make sure the onClick name matches your function!) */}
              <button 
                onClick={handlePhotoWatermarkProcess} 
                className="bg-teal-500 text-white px-10 py-4 rounded-xl font-bold shadow-lg hover:bg-teal-600 transition-transform active:scale-95 text-lg"
              >
                Remove Watermark
              </button>
            </div>
          ...
          {resultUrl && !loading && (
            <div className="animate-in zoom-in duration-300 mt-8">
              ...
              {/* 2. WE PUT THE NEW DOWNLOAD BUTTON HERE AT THE VERY BOTTOM! */}
              <button 
                onClick={() => handleForceDownload(resultUrl.startsWith('http') ? resultUrl : `https://vaniconnect-vaniconnect-api.hf.space${resultUrl}`)} 
                className="bg-teal-500 text-white px-8 py-4 rounded-xl font-bold inline-flex items-center shadow-lg hover:bg-teal-600"
              >
                <Download className="mr-2" /> Download Photo
              </button>
            </div>
        ...
        <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-lg border p-10 text-center">
          ...
          {selectedFile && !loading && !resultUrl && (
            <div>
              ...
              {/* 1. WE RESTORED THE START BUTTON HERE! */}
              <button 
                onClick={handleVideoWatermarkProcess} 
                className="bg-amber-500 text-white px-10 py-4 rounded-xl font-bold shadow-lg hover:bg-amber-600 transition-transform active:scale-95 text-lg"
              >
                Remove Watermark
              </button>
            </div>
          ...
          {resultUrl && !loading && (
            <div className="animate-in zoom-in duration-300">
              ...
              <button 
                onClick={() => handleForceDownload(resultUrl.startsWith('http') ? resultUrl : `https://vaniconnect-vaniconnect-api.hf.space${resultUrl}`)} 
                className="bg-amber-500 text-white px-8 py-4 rounded-xl font-bold inline-flex items-center shadow-lg hover:bg-amber-600"
              >
  ...
  if (activeTool === "Clip Cut Pro Editor") {
    return (
      <div className="min-h-screen bg-gray-50 p-10 font-sans">
        {/* Replace the old download button with this one */}
              <button onClick={() => handleForceDownload(resultUrl)} className="bg-purple-600 text-white px-8 py-4 rounded-xl font-bold inline-flex items-center shadow-lg hover:bg-purple-700">
        ...
        <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-lg border p-10 text-center animate-in fade-in">
            ...
            <video src={previewUrl} controls className="w-full max-h-96 mx-auto mb-6 rounded-xl shadow-md bg-black" />
             ...
             <button 
                onClick={async () => {
                  ...
                  try {
                    // 1. Point the upload to your local engine
                    const res = await axios.post(`${import.meta.env.VITE_API_URL}/api/clipcut-pro`, formData);
                    
                    if (res.data && res.data.job_id) {
                      checkJobStatus(res.data.job_id);
                    } else if (res.data && (res.data.file_name || res.data.file_url)) {
                      // 2. Point the download to your local engine
                      const finalUrl = res.data.file_url 
                          ? res.data.file_url 
                          : `${import.meta.env.VITE_API_URL}/downloads/${res.data.file_name}`;
                      setResultUrl(finalUrl);
                      setLoading(false);
                    } else {
                      alert("Backend did not return a file!");
         ...
         {resultUrl && !loading && (
            <div className="mt-8 animate-in zoom-in duration-300">
              ...
              {/* 2. THE RESULT PLAYER: This is where the fixed code was supposed to go! */}
              <video src={resultUrl} controls className="w-full max-h-96 mx-auto mb-6 rounded-xl shadow-lg bg-black" />
